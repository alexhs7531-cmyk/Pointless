...
function towerCountdown(target, tickMs, suspense, done) {
  ...
  // The show's suspense finish: ticks stretch out as the value drops
  // towards pointless. Multipliers of the base speed — at the default
  // 75ms/pt this gives roughly 1.0s for 3→2, 1.2s for 2→1 and 1.4s
  // for the final drop onto POINTLESS.
  const SUSP = { 12: 1.5, 11: 2, 10: 2.7, 9: 3.4, 8: 4.2, 7: 5.2,
                 6: 6.4, 5: 7.8, 4: 9.5, 3: 11.5, 2: 13.5, 1: 16, 0: 19 };

  const durs = [];
  for (let v = 99; v >= target; v--) {
    durs.push(base * (suspense && SUSP[v] ? SUSP[v] : 1));
  }
  const cum = [0];
