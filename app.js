...
function defaultState() {
  return {
    version: 1,
    settings: { tickMs: 75, sound: true, blur: false },
    players: [
...
function loadState() {
  try {
    ...
    s.ui = { showAll: false };
    s.settings = Object.assign({ tickMs: 75, sound: true, blur: false }, s.settings || {});
    return s;
  } catch (e) { return null; }
}
...
function towerCountdown(target, tickMs, done) {
  towerClearStates();
  towerValueSet(100);
  towerLevelSet(100);
  ...
  const speed = clamp(tickMs || state.settings.tickMs || 75, 30, 220);
  const total = steps * speed;
  let startT = null;
  let shown = 100;
  ...
  function frame(now) {
    ...
    const cur = 100 - steps * p;
    const disp = Math.max(target, Math.ceil(cur - 1e-9));
    if (disp < shown) {
      shown = disp;
      towerValueSet(shown);
      sfx.tick(shown);
...
function boardGameOver(names, totals, winner) {
  ...
  if (winner === -1) {
    ...
    boardAnswer('Final scores', names[0] + ' ' + totals[0] + '  —  ' + totals[1] + ' ' + names[1], true);
  } else {
  ...
  boardPrompt('Final: ' + names[0] + ' ' + totals[0] + '  ·  ' + names[1] + ' ' + totals[1]);
}
...
function playEvent(ev, done) {
  switch (ev.t) {
    ...
    case 'answer': {
      ...
      boardPrompt('');
      setTimeout(() => {
        towerCountdown(ev.score, ev.tickMs, () => {
          const finish = () => {
...
function overWinner() {
  const [a, b] = state.players.map(p => p.total);
  if (a === b) return -1;
  return a < b ? 0 : 1;
}
...
function recordAnswer(a) {
  ...
  const bothDone = !!(g.answers[0] && g.answers[1]);
...
function renderHostGame() {
  ...
  if (state.over) {
    ...
    box.innerHTML =
      ...
      '<div class="status-sub">' + esc(state.players[0].name) + ': ' + state.players[0].total +
      ' · ' + esc(state.players[1].name) + ': ' + state.players[1].total + '</div>' +
      '<div class="btnrow">' +
  ...
  if (!g) {
    ...
    box.innerHTML =
      ...
      '<div class="panel">' +
        ...
        '</div>' +
        '<div class="hint">Totals — ' + esc(state.players[0].name) + ': ' + state.players[0].total +
          ' · ' + esc(state.players[1].name) + ': ' + state.players[1].total + '</div>' +
      '</div>' +
      '<div class="panel">' +
        '<h4>Play next</h4>' +
...
function renderHostSettings() {
  ...
  $('#tab-settings').innerHTML =
    ...
    '<div class="panel">' +
      '<h4>Players</h4>' +
      '<div class="set-row"><label>Quizzer 1</label>' +
        '<input type="text" data-set="name0" value="' + esc(state.players[0].name) + '"></div>' +
      '<div class="set-row"><label>Quizzer 2</label>' +
        '<input type="text" data-set="name1" value="' + esc(state.players[1].name) + '"></div>' +
      '<div class="btnrow">' +
    ...
    '<div class="panel">' +
      ...
      '</div>' +
      '<div class="hint">Drag the display window to your TV and press F for fullscreen. It mirrors everything automatically (same computer &amp; browser). Click it once to enable sound; press M in either window to mute just that window.</div>' +
    '</div></div>';
}
...
function bindHost() {
  ...
  el.hostDrawer.addEventListener('change', (e) => {
    ...
    if (set === 'name0') setName(0, e.target.value);
    if (set === 'name1') setName(1, e.target.value);
